...
fn path_registration_and_listing() {
    ...
    assert!(children
        .iter()
        .any(|(p, pid)| p == "/system/service/one" && *pid == a));
    assert!(children
        .iter()
        .any(|(p, pid)| p == "/system/service/two" && *pid == b));
    ...
    assert!(all_under_two
        .iter()
        .any(|(p, pid)| p == "/system/service/two/grand" && *pid == c));
    ...
    assert!(!direct_under_service
        .iter()
        .any(|(p, _)| p == "/system/service/two/grand"));
